...
public class EjRectas {
	public static void main(String [] args) {
		...
		double base = p2.distancia(p3);				// Se calcula la distancia entre p2 y p3
		Recta rBase = new Recta(p2,p3);				// Se calcula la recta que pasa por p2 y p3
		double altura = rBase.distanciaDesde(p1);	// se calcula la distancia entre p1 y rBase
		double area = base*altura/2;				// El area es base*altura/2
		System.out.printf ("Puntos %s %s %s\n",p1,p2,p3);
		System.out.println ("area = " + area);


		Punto p4 = new Punto(1,-4);			// La perpendicular a rBase por p3
